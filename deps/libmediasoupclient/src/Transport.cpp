...
namespace mediasoupclient
{
	  ...
	  const json& appData)
	{
		...
		// May throw.创建RtpReceiverInterface相关接口，对于远端流，peerconnection可以根据远程sdp创建一个track
		auto recvResult = this->recvHandler->Receive(id, kind, rtpParameters);
		//根据rtpReceiver组装一个Consumer
	  ...
	  const nlohmann::json& appData)
	{
		...
		// This may throw.创建DataChannelInterface
