...
int MediaControl::join(const std::string& url, vi::peer_info* peer_info)
{
	...
	if (peer_info)
	{
		user_info_.reset(new user_info());
...
int MediaControl::publish(bool benable_video, bool benable_audio)
{
	auto pc = PeerConnectionClient::Create();
	enable_audio(pc, benable_audio);
	enable_video(pc, benable_video);
	publish_pcs_.push_back(std::move(pc));
	return 0;
}
int MediaControl::unpublish()
{
...
int MediaControl::enable_video(std::unique_ptr<PeerConnectionClient>& pc, bool enable)
{
...
void MediaControl::on_new_consumer(string req)
{
	...
	recv_tp->sdp->Receive(local_id, new_consumer_req->data->kind.value_or(""), rtp_param, cname, recv_id);
