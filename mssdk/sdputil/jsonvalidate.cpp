...
{
	if (!caps.is_object())
		LAZY_THROW_EXCEPTION("caps is not an object");
	...
	{
		LAZY_THROW_EXCEPTION("caps.codecs is not an array");
	}
	...
	{
		LAZY_THROW_EXCEPTION("caps.headerExtensions is not an array");
	}
...
void validateRtpCodecCapability(json& codec)
{
	...
	if (!codec.is_object())
		LAZY_THROW_EXCEPTION("codec is not an object");
	...
	if (mimeTypeIt == codec.end() || !mimeTypeIt->is_string())
		LAZY_THROW_EXCEPTION("missing codec.mimeType");
	...
	if (mimeTypeMatch.empty())
		LAZY_THROW_EXCEPTION("invalid codec.mimeType");
	...
	if (preferredPayloadTypeIt != codec.end() && !preferredPayloadTypeIt->is_number_integer())
		LAZY_THROW_EXCEPTION("invalid codec.preferredPayloadType");

	// clockRate is mandatory.
	if (clockRateIt == codec.end() || !clockRateIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing codec.clockRate");
	...
	for (auto it = parametersIt->begin(); it != parametersIt->end(); ++it)
	{
		...
		if (!value.is_string() && !value.is_number() && value != nullptr)
			LAZY_THROW_EXCEPTION("invalid codec parameter");

		// Specific parameters validation.
		if (key == "apt")
		{
			if (!value.is_number_integer())
				LAZY_THROW_EXCEPTION("invalid codec apt parameter");
		}
	}
...
{
	if (!fb.is_object())
		LAZY_THROW_EXCEPTION("fb is not an object");
	...
	if (typeIt == fb.end() || !typeIt->is_string())
		LAZY_THROW_EXCEPTION("missing fb.type");
...
{
	if (!ext.is_object())
		LAZY_THROW_EXCEPTION("ext is not an object");
	...
	if (kindIt == ext.end() || !kindIt->is_string())
		LAZY_THROW_EXCEPTION("missing ext.kind");

	kindIt = ext.find("kind");
	std::string kind = kindIt->get<std::string>();

	if (kind != "audio" && kind != "video")
		LAZY_THROW_EXCEPTION("invalid ext.kind");

	// uri is mandatory.
	if (uriIt == ext.end() || !uriIt->is_string() || uriIt->get<std::string>().empty())
		LAZY_THROW_EXCEPTION("missing ext.uri");

	// preferredId is mandatory.
	if (preferredIdIt == ext.end() || !preferredIdIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing ext.preferredId");

	// preferredEncrypt is optional. If unset set it to false.
	if (preferredEncryptIt != ext.end() && !preferredEncryptIt->is_boolean())
	{
		RTC_LOG(LS_INFO) << "preferredEncryptIt typename:" << preferredEncryptIt->type_name();
		LAZY_THROW_EXCEPTION("invalid ext.preferredEncrypt");
	}
	...
	if (directionIt != ext.end() && !directionIt->is_string())
		LAZY_THROW_EXCEPTION("invalid ext.direction");
...
{
	if (!params.is_object())
		LAZY_THROW_EXCEPTION("params is not an object");
	...
	{
		LAZY_THROW_EXCEPTION("params.mid is not a string");
	}

	// codecs is mandatory.
	if (codecsIt == params.end() || !codecsIt->is_array())
		LAZY_THROW_EXCEPTION("missing params.codecs");
	...
	{
		LAZY_THROW_EXCEPTION("params.headerExtensions is not an array");
	}
	...
	{
		LAZY_THROW_EXCEPTION("params.encodings is not an array");
	}
	...
	{
		LAZY_THROW_EXCEPTION("params.rtcp is not an object");
	}
...
void validateRtpCodecParameters(json& codec)
{
	...
	if (payloadTypeIt == codec.end() || !payloadTypeIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing codec.payloadType");
...
void validateRtpHeaderExtensionParameters(json& ext)
{
	...
	if (idIt == ext.end() || !idIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing ext.id");

	// encrypt is optional. If unset set it to false.
	if (encryptIt != ext.end() && !encryptIt->is_boolean())
		ext["encrypt"] = false;
		//LAZY_THROW_EXCEPTION("invalid ext.encrypt");
	...
	for (auto it = parametersIt->begin(); it != parametersIt->end(); ++it)
	{
		...
		if (!value.is_string() && !value.is_number())
			LAZY_THROW_EXCEPTION("invalid header extension parameter");
	}
}
...
void validateRtpEncodingParameters(json& encoding)
{
	if (!encoding.is_object())
		LAZY_THROW_EXCEPTION("encoding is not an object");
	...
	if (ssrcIt != encoding.end() && !ssrcIt->is_number_integer())
		LAZY_THROW_EXCEPTION("invalid encoding.ssrc");

	// rid is optional.
	if (ridIt != encoding.end() && (!ridIt->is_string() || ridIt->get<std::string>().empty()))
	{
		LAZY_THROW_EXCEPTION("invalid encoding.rid");
	}

	// rtx is optional.
	if (rtxIt != encoding.end() && !rtxIt->is_object())
	{
		LAZY_THROW_EXCEPTION("invalid encoding.rtx");
	}
	else if (rtxIt != encoding.end())
	{
		...
		if (rtxSsrcIt == rtxIt->end() || !rtxSsrcIt->is_number_integer())
			LAZY_THROW_EXCEPTION("missing encoding.rtx.ssrc");
	}
	...
	if (
		...
		// clang-format on
	{
		LAZY_THROW_EXCEPTION("invalid encoding.scalabilityMode");
	}
}
...
{
	if (!rtcp.is_object())
		LAZY_THROW_EXCEPTION("rtcp is not an object");
	...
	if (cnameIt != rtcp.end() && !cnameIt->is_string())
		LAZY_THROW_EXCEPTION("invalid rtcp.cname");
...
void validateSctpCapabilities(json& caps)
{
	...
	if (numStreamsIt == caps.end() || !numStreamsIt->is_object())
		LAZY_THROW_EXCEPTION("missing caps.numStreams");
...
void validateNumSctpStreams(json& numStreams)
{
	if (!numStreams.is_object())
		LAZY_THROW_EXCEPTION("numStreams is not an object");
	...
	if (osIt == numStreams.end() || !osIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing numStreams.OS");

	// MIS is mandatory.
	if (misIt == numStreams.end() || !misIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing numStreams.MIS");
}
...
void validateSctpParameters(json& params)
{
	...
	if (portIt == params.end() || !portIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing params.port");

	// OS is mandatory.
	if (osIt == params.end() || !osIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing params.OS");

	// MIS is mandatory.
	if (misIt == params.end() || !misIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing params.MIS");

	// maxMessageSize is mandatory.
	if (maxMessageSizeIt == params.end() || !maxMessageSizeIt->is_number_integer())
	{
		LAZY_THROW_EXCEPTION("missing params.maxMessageSize");
	}
}
...
void validateSctpStreamParameters(json& params)
{
	...
	if (streamIdIt == params.end() || !streamIdIt->is_number_integer())
		LAZY_THROW_EXCEPTION("missing params.streamId");
	...
	{
		LAZY_THROW_EXCEPTION("cannot provide both maxPacketLifeTime and maxRetransmits");
	}
	...
	if (
		...
		// clang-format on
	{
		LAZY_THROW_EXCEPTION("cannot be ordered with maxPacketLifeTime or maxRetransmits");
	}
	// clang-format off
	else if (
		!orderedGiven &&
...
void validateIceCandidate(json& params)
{
	...
	auto ipIt           = params.find("address"); //("ip");
	auto protocolIt = params.find("protocol");
	auto portIt = params.find("port");
	...
	if (protocolMatch.empty())
		LAZY_THROW_EXCEPTION("invalid params.protocol");
	...
	{
		LAZY_THROW_EXCEPTION("missing params.type");
	}
	...
	if (typeMatch.empty())
		LAZY_THROW_EXCEPTION("invalid params.type");
}
...
{
	if (!params.is_array())
		LAZY_THROW_EXCEPTION("params is not an array");
...
void validateDtlsFingerprint(json& params)
{
	...
	{
		LAZY_THROW_EXCEPTION("missing params.algorithm");
	}

	// foundation is mandatory.
	if (valueIt == params.end() || (!valueIt->is_string() || valueIt->get<std::string>().empty()))
	{
		LAZY_THROW_EXCEPTION("missing params.value");
	}
}
...
void validateDtlsParameters(json& params)
{
	...
	{
		LAZY_THROW_EXCEPTION("missing params.role");
	}
...
{
	// This may throw.
	//std::string localstr = localCaps.dump();
	//std::string remotestr = remoteCaps.dump();
...
const json generateProbatorRtpParameters(const json& videoRtpParameters)
{
	...
	// This may throw.
	validateRtpParameters(validatedRtpParameters);
...
nlohmann::json reduceCodecs(nlohmann::json& codecs, const nlohmann::json* capCodec)
{
	...
	else
	{
		...
		if (filteredCodecs.size() == 0)
			LAZY_THROW_EXCEPTION("no matching codec found");
	}
