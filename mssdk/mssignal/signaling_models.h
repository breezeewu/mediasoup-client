...
// mediasoup signal
#define MS_MEDIASOUP_VERSION                   "mediasoup-version"
#define MS_GET_ROUTER_RTP_CAPABILITIES         "getRouterRtpCapabilities"
#define MS_CREATE_WEBRTC_TRANSPORT             "createWebRtcTransport"
#define MS_JOIN                                "join"
#define MS_NEW_CONSUMER                        "newConsumer"
#define MS_NEW_DATA_CONSUMER                   "newDataConsumer"
#define MS_CONNECT_WEBRTC_TRANSPORT            "connectWebRtcTransport"
#define MS_PRODUCE                             "produce"
...
namespace signaling {
    ...
    struct CommonHeader {
        ...
        FIELDS_MAP("request", request, "response", response, "notification", notification,"id", id, "method", method, "ok", ok);
        ...
        MSGTYPE msgType()
        {
            ...
            else if (response.has_value() && response.value())
            {
                return MSG_TYPE_RESPONSE;
            }
            else if (notification.has_value() && notification.value())
            {
...
std::shared_ptr<Type> from_json_string(const std::string& str)
{
    std::string err;
    auto request = fromJsonString<Type>(str, err);
    if (!err.empty()) 
    {
        RTC_LOG(LS_ERROR) << "parser json:" << str << "failed, reason:" << err;
        LAZY_THROW_EXCEPTION("parser json:%s failed, reason:%s", str.c_str(), err.c_str());
    }
    return request;
}
}
